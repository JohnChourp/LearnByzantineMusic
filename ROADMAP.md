...
- Επεκτάθηκε η κλίμακα της σελίδας `8 Ήχοι` σε 3 οκτάβες ανά ήχο (`Πα, → Πα΄΄` ή `Νη, → Νη΄΄`) με 22 φθόγγους/21 διαστήματα και touch playback σε όλο το εύρος.
- Προστέθηκε ρυθμιζόμενη μεταφορά βάσης στη σελίδα `8 Ήχοι` με slider `-12..+12` μόρια και κουμπί `Επαναφορά`, όπου κάθε ήχος αποθηκεύει μόνιμα τη δική του τιμή βάσης, ενώ αποθηκεύεται και ο τελευταίος επιλεγμένος ήχος του selector.
- Προστέθηκε επιλογέας ηχοχρώματος στη σελίδα `8 Ήχοι` και κατόπιν αξιολόγησης περιορίστηκε στα 3 τελικά συνθετικά presets (`Καθαρός`, `Μαλακός`, `Κρυστάλλινο`) με μόνιμη αποθήκευση τελευταίας επιλογής χρήστη.
...
- Προστέθηκε νέα σελίδα `Σημειώσεις` με local-first Room αποθήκευση (`τίτλος + κείμενο`), αναζήτηση/ταξινόμηση, auto-save + χειροκίνητο save και σύνδεση από την αρχική οθόνη.
- Προστέθηκε υποχρεωτική επιλογή φακέλου backup/sync για `Σημειώσεις` στην πρώτη είσοδο μέσω SAF, με μόνιμη πρόσβαση φακέλου.
- Προστέθηκε full JSON snapshot export σε κάθε save σημείωσης με απεριόριστα backup αρχεία και υποστήριξη `Εξαγωγή τώρα` + `Import backup (replace-all)`.
- Προστέθηκε pending resync μηχανισμός για `Σημειώσεις`: όταν αποτυγχάνει write στον φάκελο backup, το snapshot αποθηκεύεται τοπικά και γίνεται retry σε επόμενο save ή με χειροκίνητο επανασυγχρονισμό.
...
- [ ] Φάση 2 ήχου για `8 Ήχοι`: επιλογή 1-2 τελικών ηχοχρωμάτων και ενσωμάτωση sample-based CC0/Public Domain assets με fallback στον υπάρχοντα synth engine.
