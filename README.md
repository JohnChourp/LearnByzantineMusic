...
Προστέθηκε σελίδα `Ηχογραφήσεις` με μεγάλη εγγραφή μικροφώνου, παύση/συνέχεια, σταμάτημα, recent feed ηχογραφήσεων και pagination (`10/10`) από όλους τους φακέλους/υποφακέλους.
...
Προστέθηκε ξεχωριστή σελίδα `Διαχείριση ηχογραφήσεων` για πλοήγηση φακέλων, δημιουργία φακέλου, drag/drop μετακίνηση με επιβεβαίωση source/target path και inline rename/delete.
...
- Πατώντας `Ηχογραφήσεις`, ανοίγει σελίδα εγγραφής με controls (`Έναρξη/Παύση/Συνέχεια/Σταμάτημα`) και recent λίστα ηχογραφήσεων από όλο το SAF tree.
...
- Με `Σταμάτημα` το app αποθηκεύει την εγγραφή στο επιλεγμένο format (`FLAC` default) και ενημερώνει το recent feed.
...
- Με `Διαχείριση ηχογραφήσεων` ανοίγει ξεχωριστή σελίδα όπου εμφανίζονται φάκελοι + audio files του τρέχοντος path, επιτρέπεται δημιουργία φακέλου, drag/drop μετακίνηση με επιβεβαίωση και inline rename/delete.
...
- Το recent feed συνεχίζει να δείχνει εγγραφές από το ίδιο SAF root.
...
- Η μετακίνηση βασίζεται σε SAF `moveDocument` και ορισμένοι providers μπορεί να μη την υποστηρίζουν για όλα τα paths.
...
- `RecordingsActivity`, `layout_recordings.xml` και `list_item_recording.xml`: σελίδα ηχογραφήσεων με SAF folder access, controls εγγραφής (`start/pause/stop`), recent feed από όλο το tree με pagination (`10/10`) και inline μετονομασία/διαγραφή.
